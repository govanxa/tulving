...
- **When the user states something durable** (a decision, fact, plan, or useful
  observation), call `memory_store`:
    ...
    - `key`: a stable, prefixed handle, e.g. `decision:api-scheme`, `fact:rate-limit`
      (avoid `auth`/`token`/`secret`/`password`/`key`/`credential` in a key — Tulving
      masks the content of a key that looks like it names a secret)
...
- **When the user states something durable**, call `memory_store`:
    - `type`: `decision` | `fact` | `plan` | `observation`
    - `key`: prefix with the project, e.g. `proj-acme:decision:api-scheme`
      (avoid `auth`/`token`/`secret`/`password`/`key`/`credential` in a key — those get masked)
